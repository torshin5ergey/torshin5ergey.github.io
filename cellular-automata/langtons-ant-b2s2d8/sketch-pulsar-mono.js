...
function setup() {
  ...
  WIDTH = 150; // CA field width
  HEIGHT = 150; // CA field height
  // Single cell size (px)
  ...
  cells = generateCenteredSquare(cells, 5);
  noStroke();
}
...
/**
 * Generate a centered square with specified dimensions and generations value.
 * @param {Array<Array<number>>} array - The array representing the current state of cells.
 * @param {number} squareSize - The size of the square to generate.
 * @returns {Array<Array<number>>} - The array with the centered square.
 */
function generateCenteredSquare(array, squareSize) {
  let startX = Math.floor((WIDTH - squareSize) / 2);
  let startY = Math.floor((HEIGHT - squareSize) / 2);

  for (let i = startX; i < startX + squareSize; i++) {
    for (let j = startY; j < startY + squareSize; j++) {
      array[i][j] = generations;
    }
  }
  return array;
}
...
function mouseClicked() {
  if (mouseButton === LEFT) {
    cells = generateRandomCells(0);
    cells = generateCenteredSquare(cells, random(3, 9));
  }
}
